...
//        '@' => 'file',
    ...
    protected function filterData(array &$data, array $schema, Validation &$validation, $path = '') {
        foreach ($data as $key => $val) {
            ...
            $errorMessage = sprintf($this->translate('Unexpected parameter: %1$s.'), $path.$key);
    ...
    public function requireOneOf(array $fieldnames, $count = 1) {
        $result = $this->addValidator('*', function ($data, Validation $validation) use ($fieldnames, $count) {
            ...
            if ($count === 1) {
                $message = sprintf($this->translate('One of %s are required.'), implode(', ', $messageFields));
            } else {
                $message = sprintf($this->translate('%1$s of %2$s are required.'), $count, implode(', ', $messageFields));
            }
    ...
    protected function validateField(&$value, array $field, Validation $validation) {
        ...
        if (!$validType) {
            ...
            $validation->addError(
                ...
                [
                    'type' => $type,
                    'message' => sprintf($this->translate('%1$s is not a valid %2$s.'), $path, $type),
                    'status' => 422
                ]
            );
        }
